...
describe 'Merchants API' do
  ...
  it 'retuns an amount of merchants ranked by total number of items sold' do
    ...
    expect(merchants_by_sales[0][:attributes][:name]).to eq(merchant3.name)
    expect(merchants_by_sales[1][:attributes][:name]).to eq(merchant4.name)
    expect(merchants_by_sales[2][:attributes][:name]).to eq(merchant1.name)
  end

  it 'retuns an error if no quantity is given' do
    merchant1 = create(:merchant)
    merchant2 = create(:merchant)
    merchant3 = create(:merchant)
    merchant4 = create(:merchant)
    ...
    get api_v1_merchants_most_items_path({ quantity: nil })
  ...
  it 'has a sad path with no data given' do
    ...
    matching_merchants = JSON.parse(response.body, symbolize_names: true)
    expect(matching_merchants[:data]).to eq({})
  end

  it 'returns something when no merchant' do
    (create_list :merchant, 20)
  ...
  it 'can find no merchants based on search criteria' do
    (create_list :merchant, 3)
  ...
  it 'can find no merchants based on ' do
    (create_list :merchant, 3)

    get api_v1_merchants_find_all_path({ name: nil })
    matching_merchants = JSON.parse(response.body, symbolize_names: true)
    expect(matching_merchants[:data]).to eq([])
  end
end
